...
| 2026-07-11 | Rimosso il fallback 72/36 (falsava il calcolo): scrape_campi_api() ora ricava il par reale invertendo la formula WHS del Playing Handicap che l'API Federgolf calcola per HCP 18.0 (row[4] di course-handicap, verificato via MCP Playwright su percorsi con e senza "Par NN" nel nome, es. "Prime Nove"); se il par resta NULL in DB, /api/simulazione_hcp e la pagina Handicap di gioco ritornano/mostrano un errore esplicito invece di calcolare con un default sbagliato |
| 2026-07-11 | simulazione.html: rimossi i due grafici Chart.js, sostituiti con una tabella ordinata per SD crescente (colonne Data/Gara/SD/★ best8) che usa gli stessi sd_list e best8_indices già restituiti da /api/simulazione_hcp; riga della gara simulata evidenziata in ambra con etichetta "SIMULATA"; riepilogo (media migliori 8 = nuovo HCP, HCP attuale, variazione) spostato sotto la tabella |
| 2026-07-11 | simulazione.html: la tabella ora ordina per data decrescente (più recente in alto, gara simulata sempre in cima) invece che per SD crescente; i migliori 8 restano evidenziati con ★/sfondo verde indipendentemente dalla posizione in tabella |
| 2026-07-11 | Tabella simulazione: aggiunte colonne Formula e Score (stbl "pt" se presente, altrimenti ags "colpi") — /api/simulazione_hcp ora seleziona anche formula/stbl/ags dalla tabella gare; la riga della gara simulata mostra tipo e valore inseriti dall'utente (Stableford/Medal + punti o colpi digitati) |
